/*  
    ...
    to have the robot go around a course of markers.
...
enum FSM {Boot, Seek, Ping1, Stop1, Ping2, Turn, Stop2, Forward, Stop3, Turn90, Stop4, Circle, Ping3, Semi, Done};
FSM state      = Seek;
FSM nextState  = Seek;
bool firstLoop = true;
...
// - - - - - - - - - - - - - - - - - - - Finite State Machine - - - - - - - - - - - - - - - - - - - - - -   
  // Finite state machine (Boot, Seek, Ping1, Stop1, Ping2, Turn, Stop2, Forward, Stop3, Turn90, Stop4, Circle, Ping3, Done)
  switch(state){
    case Boot:
      if(pingPI() == true){
        nextState = Seek;
    ...
    case Circle:
      if(firstLoop){
        circleRobot(1.2, 20000);
        pingPI();
        firstLoop = false;
        FSMTimer = 1000;
      }else if(FSMTimer >= 300){
        pingPI();
      }else if(FSMTimer <= 0){
        nextState = Ping3;     
      }
